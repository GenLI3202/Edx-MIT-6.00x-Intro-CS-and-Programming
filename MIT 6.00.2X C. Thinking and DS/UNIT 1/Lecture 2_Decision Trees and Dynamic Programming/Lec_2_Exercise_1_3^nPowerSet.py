# -*- coding: utf-8 -*-
"""
Created on Fri Apr  3 22:40:03 2020
...
# Task Description:
#
# For the following problem, consider the following way to 
# write a power set generator. The number of possible combinations to 
# put n items into one bag is  2n . 
...
# Each combination should be given as a tuple of two lists, the 
# first being the items in bag1, and the second being the items in bag2.
    ...
    for i in range(3**N):
        ...
        for j in range(N):
            ...
            if (i // (3**j)) % 3 == 1:
                ...
                combo_Bag2.append(items[j])
        yield combo = (combo_Bag1,combo_Bag2)


# Functions of test method asi
import random
class Item(object):
    ...
    def __str__(self):
        return '<' + self.name + ', ' +str(self.value) + ', '\
                    + str(self.weight) + '>'
def buildItems():
    return [Item(name, val, weight) for name,val, weight in (('clock', 175, 10), 
                                                     ('painting', 90, 9),
                                                     ('radio', 20, 4),
                                                     ('vase', 50, 2),
                                                     ('book', 10, 1),
