...
def retire(monthly, rate, terms):
    ...
    for i in range(terms):
        base += [i]
        savings += [savings[-1]*(1 + monthlyRate) + monthly]    #savings[1] is the latest amount in my acount
        # print('the '+ str(i) + 'th savings is: ', savings[i])
    return base, savings
...
def displayRetirementWithMonthsAndRates(monthlies, rates, terms):
    ...
    plt.clf()
    plt.xlim(30*12, 40*12)          # set out the x-limit: only observe the last 10 years.
    for monthly in monthlies:
        for rate in rates:
            xvals, yvals = retire(monthly, rate, terms)
            plt.plot(xvals, yvals,
...
def displayRetirementWithMonthsAndRates(monthlies, rates, terms):
    ...
    for i in range(len(monthlies)):
        ...
        for j in range(len(rates)):
            ...
            xvals, yvals = retire(monthly, rate, terms)
            plt.plot(xvals, yvals, monthLabel+rateLabel,            # "mothLabel+rateLabel": crate label for plot
                    label = 'retire:'+str(monthly)+ ':' + str(int(rate*100)))
            plt.legend(loc = 'upper left')
